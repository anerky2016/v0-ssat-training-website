...
export default function GenerateSynonymsPage() {
  ...
  const handleGenerate = async () => {
    ...
    if (mode === 'batch' && !wordList.trim()) {
      setError("Please enter a list of words")
      return
    }
    ...
    try {
      if (mode === 'single') {
        ...
        if (words.length === 0) {
          throw new Error('No valid words found in the list')
        }
        ...
        console.log(`Generated ${allQuestions.length} questions for ${words.length} words`)
      }
    } catch (err: any) {
            ...
            <div className="mx-auto max-w-5xl">
              ...
              <div className="mb-8">
                ...
                <p className="text-lg text-muted-foreground">
                  Use AI to create SSAT-style synonym questions for any vocabulary word
                </p>
              </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {mode === 'single' ? (
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1">
                            Helps generate more accurate questions
                          </p>
                        </div>
                      ...
                      <div>
                        ...
                        <Textarea
                          id="wordList"
                          placeholder="Enter one word per line, e.g.:&#10;eloquent&#10;resilient&#10;meticulous&#10;tenacious&#10;pragmatic"
                          value={wordList}
                    ...
                    <div>
                      <Label htmlFor="count">
                        {mode === 'single' ? 'Number of Questions' : 'Questions Per Word'}
                      </Label>
                      ...
                      {mode === 'batch' && (
                        <p className="text-xs text-muted-foreground mt-1">
                          Total: {questionCount} × {wordList.split('\n').filter(w => w.trim().length > 0).length} words = {questionCount * wordList.split('\n').filter(w => w.trim().length > 0).length} questions
                        </p>
                      )}
                    </div>
                    ...
                    <Button
                      ...
                      disabled={isGenerating || (mode === 'single' ? !word.trim() : !wordList.trim())}
                    ...
                    >
                      {isGenerating ? (
                        ...
                        <>
                          ...
                          {mode === 'single' ? 'Generate Questions' : 'Generate All Questions'}
                        </>
                      )}
                    </Button>
                  </CardContent>
                    ...
                    {generatedQuestions.length === 0 ? (
                      <div className="text-center py-12">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          Enter a word and click "Generate Questions"
                        </p>
                      </div>
                    ) : (
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm mb-4">
                    <div>
                      ...
                      <p className="text-muted-foreground">
                        Generate questions for a single word with details, or batch-generate for multiple words at once.
                      </p>
                    </div>
                    <div>
                  ...
                  <div className="p-3 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-950/20 dark:to-pink-950/20 rounded-lg border border-purple-200 dark:border-purple-800">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <div className="font-semibold text-purple-900 dark:text-purple-100 mb-1">Batch Generation Tip</div>
                        <p className="text-xs text-purple-800 dark:text-purple-200">
                          Paste a list of vocabulary words (one per line) to generate multiple questions at once. Perfect for preparing study materials for entire word lists!
                        </p>
                      </div>
                    </div>
                  </div>
