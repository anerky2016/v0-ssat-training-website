...
// Read the Level 6 vocabulary JSON file
const dataPath = path.join(__dirname, '../data/wordly-wise-level-6.json');
const vocabularyData = JSON.parse(fs.readFileSync(dataPath, 'utf8'));

console.log(`📚 Found ${vocabularyData.words.length} words to generate kid-friendly examples for (ages 10-11)\n`);
...
Requirements:
- Sentences must be appropriate for 10-11 year old children (more advanced than 8-9 year olds)
- Use vocabulary that a fourth or fifth grader would understand
- Keep sentences medium to longer length (18-35 words maximum)
- Use contexts children can relate to: school, home, friends, hobbies, sports, nature, science, history, books, games, technology, current events, etc.
- Make the examples clear, educational, and engaging
- Each sentence should naturally use the word "${word}"
- Can include more complex ideas and longer sentences than for younger children
- Return ONLY the sentences, one per line, no numbering or bullets

Example format:
The cat likes to play with the ball in the backyard after school, and it always makes me smile.
My friend shared her toy with me at recess, and we had so much fun together that we forgot about the time.
We had a great time at the park today with our families and friends, playing games and enjoying the beautiful weather.`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a helpful teacher who creates age-appropriate examples for children learning vocabulary. You specialize in creating examples for 10-11 year old children.'
        },
        {
          role: 'user',
          content: prompt
    ...
    const examples = content
      ...
      .filter(line => {
        ...
        return cleaned.length > 18 && cleaned.length < 300;
      })
