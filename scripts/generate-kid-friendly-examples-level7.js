...
// Read the Level 7 vocabulary JSON file
const dataPath = path.join(__dirname, '../data/wordly-wise-level-7.json');
const vocabularyData = JSON.parse(fs.readFileSync(dataPath, 'utf8'));

console.log(`📚 Found ${vocabularyData.words.length} words to generate kid-friendly examples for (ages 11-12)\n`);
...
async function generateKidFriendlyExamples(word, meaning, partOfSpeech) {
  try {
    const prompt = `Generate 3-5 example sentences for the word "${word}" (${partOfSpeech}).

Meaning: ${meaning}

Requirements:
- Sentences must be appropriate for 11-12 year old children (more advanced than 10-11 year olds)
- Use vocabulary that a fifth or sixth grader would understand
- Keep sentences longer and more sophisticated (20-40 words maximum)
- Use contexts children can relate to: school, home, friends, hobbies, sports, nature, science, history, books, games, technology, current events, social issues, etc.
- Make the examples clear, educational, engaging, and thought-provoking
- Each sentence should naturally use the word "${word}"
- Can include complex ideas, longer sentences, and more sophisticated vocabulary than for younger children
- Return ONLY the sentences, one per line, no numbering or bullets

Example format:
The cat likes to play with the ball in the backyard after school, and it always makes me smile when I see how happy it is.
My friend shared her toy with me at recess, and we had so much fun together that we forgot about the time and almost missed our next class.
We had a great time at the park today with our families and friends, playing games and enjoying the beautiful weather while we discussed our plans for the upcoming summer vacation.`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a helpful teacher who creates age-appropriate examples for children learning vocabulary. You specialize in creating examples for 11-12 year old children.'
        },
        {
          role: 'user',
          content: prompt
    ...
    const examples = content
      ...
      .filter(line => {
        ...
        return cleaned.length > 20 && cleaned.length < 350;
      })
