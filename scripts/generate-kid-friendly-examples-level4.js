...
console.log(`📚 Found ${vocabularyData.words.length} words to generate kid-friendly examples for (ages 7-8)\n`);
...
Requirements:
- Sentences must be appropriate for 7-8 year old children (slightly more advanced than 6-7 year olds)
- Use vocabulary that a second or third grader would understand
- Keep sentences short to medium length (12-25 words maximum)
- Use contexts children can relate to: school, home, friends, toys, pets, family, games, sports, hobbies, nature, etc.
- Make the examples clear and easy to understand
- Each sentence should naturally use the word "${word}"
- Can include slightly more complex ideas than for younger children
- Return ONLY the sentences, one per line, no numbering or bullets

Example format:
The cat likes to play with the ball in the backyard.
My friend shared her toy with me at recess.
We had fun at the park today with our families.`;
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a helpful teacher who creates age-appropriate examples for children learning vocabulary. You specialize in creating examples for 7-8 year old children.'
        },
        {
          role: 'user',
          content: prompt
    ...
    const examples = content
      ...
      .filter(line => {
        ...
        return cleaned.length > 12 && cleaned.length < 200;
      })
